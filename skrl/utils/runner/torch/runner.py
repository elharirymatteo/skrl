...
class Runner:
    ...
    def _component(self, name: str) -> Type:
        ...
        elif name in ["ppo_cat", "ppo_cat_default_config"]:
            from skrl.agents.torch.ppo import PPO_CAT, PPO_CAT_DEFAULT_CONFIG
            component = PPO_CAT_DEFAULT_CONFIG if "default_config" in name else PPO_CAT
    ...
    def _generate_agent(
        ...
        elif agent_class in ["a2c", "cem", "ddpg", "ddqn", "dqn", "ppo", "ppo_cat", "rpo", "sac", "td3", "trpo"]: # CaT: added "ppo-cat"
            agent_id = possible_agents[0]
            agent_cfg = self._component(f"{agent_class}_DEFAULT_CONFIG").copy()
            agent_cfg.update(self._process_cfg(cfg["agent"]))
